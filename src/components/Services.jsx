      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-panel border border-blue-500/40 text-blue-400 text-xs font-mono font-bold uppercase tracking-widest mb-4 shadow-[0_0_15px_rgba(59,130,246,0.25)]">
            <i className="ri-radar-line text-sm text-blue-400 animate-spin" style={{ animationDuration: '6s' }}></i>
            FLIGHT CAPABILITY & SERVICES
          </div>
          <h2 className="font-display font-black text-3xl sm:text-5xl text-white tracking-tight">
            Comprehensive Aerial & <span className="bg-gradient-to-r from-white via-blue-300 to-blue-500 bg-clip-text text-transparent text-glow-blue">Media Solutions</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {servicesList.map((srv, idx) => (
            <div 
              key={idx} 
              className="glass-panel-cinematic p-8 rounded-3xl border border-white/12 glass-panel-hover group relative overflow-hidden shadow-2xl"
            >
              {/* Subtle background glow */}
              <div className="absolute top-0 right-0 w-36 h-36 bg-blue-600/5 rounded-full blur-2xl group-hover:bg-blue-600/20 transition-all duration-500"></div>
              
              <div className="flex items-center justify-between mb-6">
                <div className="w-14 h-14 rounded-2xl bg-obsidian-900 border border-white/15 flex items-center justify-center text-blue-400 group-hover:border-blue-400 group-hover:scale-110 transition-all duration-300 shadow-[0_0_20px_rgba(59,130,246,0.2)]">
                  <i className={`${srv.icon} text-3xl text-blue-400`}></i>
                </div>
                <span className="px-3 py-1 rounded-md text-[10px] font-mono font-bold bg-obsidian-950 text-slate-200 border border-blue-500/30 tracking-wider">
                  {srv.tag}
                </span>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-white/10 flex items-center justify-between text-xs font-mono font-semibold text-blue-400">
                <span className="flex items-center gap-1.5">
                  <i className="ri-checkbox-circle-fill text-blue-400 text-base"></i>
                  <span>Digital 4K Delivery</span>
                </span>
                <i className="ri-arrow-right-up-line text-lg text-slate-400 group-hover:text-blue-400 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all"></i>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 p-6 rounded-3xl glass-panel-cinematic border border-white/15 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
          <div className="flex items-center gap-4 text-left">
            <div className="w-12 h-12 rounded-2xl bg-blue-500/10 border border-blue-500/30 text-blue-400 flex items-center justify-center flex-shrink-0 shadow-[0_0_20px_rgba(59,130,246,0.25)]">
              <i className="ri-map-pin-2-fill text-2xl"></i>
            </div>
            <div>
              <div className="text-white font-bold text-base font-display">Regional Base: Newcastle Upon Tyne (NE3)</div>
              <div className="text-slate-300 text-xs font-mono">Operating across Newcastle, Sunderland, Durham & NE3 Postcodes</div>
            </div>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            {['Newcastle Upon Tyne', 'Sunderland', 'Durham', 'NE3 Postcodes', 'Tyne & Wear'].map((loc, i) => (
              <span key={i} className="px-3.5 py-1.5 rounded-xl bg-obsidian-900 border border-white/15 text-slate-200 text-xs font-mono font-medium flex items-center gap-1.5 hover:border-blue-500/40 transition-colors">
                <i className="ri-flight-takeoff-line text-blue-400"></i>
                {loc}
              </span>
            ))}
          </div>
